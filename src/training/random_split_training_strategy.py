"""Random split training strategy for single and multi-fold cross-validation."""
...
class RandomSplitTrainingStrategy(BaseTrainingStrategy):
    ...
    def train_and_evaluate_model(
        ...
        """Train and evaluate model across one or multiple folds."""
        ...
        for fold_idx, data_fold in enumerate(
            self._iter_data_folds(dataset_input), start=1
        ):
            (
                dims,
                _train_data,
    ...
    def _train_and_evaluate_single_fold(
        ...
        """Run one fold end-to-end and return test metrics."""
        split_name = str(strategy_creator.split_type).replace("_", " ").title()
        logging.info("%s", "=" * 60)
        logging.info(
            "Starting Training (%s Split) | Fold %d",
            split_name,
            fold_idx,
        )
        logging.info("%s", "=" * 60)
    ...
    def _unpack_fold_data(data_fold, fold_idx: int):
        """Validate and unpack random-split fold tuple."""
    ...
    def _iter_data_folds(self, dataset_input):
        """Yield one or multiple folds from dataset input."""
    ...
    def _resolve_ranking_group_mode(split_type: str, ranking_group_mode: str) -> str:
        ...
        default_mode = "drug" if split_type in {"random", "cross_domain"} else "cell"
        if mode == "auto":
            return default_mode
